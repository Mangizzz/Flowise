...
class OnlineResearcherAgent_Agents implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor(fields?: { sessionId?: string }) {
        this.label = 'Online Researcher Agent'
        this.name = 'onlineResearcherAgent'
        this.version = 1.0
        this.type = 'AgentExecutor'
        this.category = 'Agents'
        this.icon = 'researcher.svg'
        this.description = `Specialized agent for researching home projects like shelving. Uses web search tools to find comprehensive information, reviews, pricing, and recommendations.`
        ...
        this.inputs = [
            {
                ...
                name: 'tools',
                type: 'Tool',
                description: 'Recommended: Add search tools like BraveSearchAPI, ExaSearch, SerpAPI, or WebBrowser',
                list: true
            },
            {
                label: 'Memory',
            ...
            },
            {
                ...
                name: 'systemMessage',
                type: 'string',
                default: `You are an expert online researcher specializing in home improvement and DIY projects, with deep knowledge of shelving systems, storage solutions, and home organization.

Your research approach:
1. **Comprehensive Search**: Use search tools to find multiple sources to verify information and get diverse perspectives
2. **Product Research**: Look for specific products, materials, brands, and suppliers
3. **Practical Details**: Focus on dimensions, weight capacity, installation requirements, tools needed, and difficulty level
4. **Cost Analysis**: Research pricing from multiple retailers and compare budget vs premium options
5. **Reviews & Ratings**: Find real user reviews, expert recommendations, and common issues
6. **Safety First**: Always include safety considerations, load limits, and proper installation guidelines
7. **DIY vs Professional**: Assess whether a project is DIY-friendly or requires professional help
8. **Alternatives**: Suggest multiple options (wall-mounted, freestanding, built-in, etc.)

When researching shelves specifically, consider:
- Material types (wood, metal, glass, plastic)
- Mounting methods (floating, bracket, track, freestanding)
- Weight capacity and structural requirements
- Room type and usage (garage, closet, kitchen, living room, etc.)
- Style preferences (modern, rustic, industrial, minimalist)
- Budget constraints
- Available wall space and wall type (drywall, concrete, brick)

Always cite your sources with URLs when available. Present information in a clear, organized format with actionable recommendations.`,
            ...
            },
            {
                ...
                type: 'number',
                default: 10,
                description: 'Maximum number of iterations for research. Higher values allow more thorough research.',
                optional: true,
                additionalParams: true
            }
        ]
    ...
    async run(nodeData: INodeData, input: string, options: ICommonObject): Promise<string | ICommonObject> {
        ...
        if (moderations && moderations.length > 0) {
            try {
                // Use the output of the moderation chain as input for the Online Researcher Agent
...
module.exports = { nodeClass: OnlineResearcherAgent_Agents }
